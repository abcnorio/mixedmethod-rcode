...
| Content | Script filename | chap | chapno |
| --- | --- | --- | --- |
| xxxxxxxxxxxxxxxxxxxxxxxxxxxxx | ADDON_ladyfisher_genfunc_bayesT.r |  |  |
|  | ADDON_Lindley-Paradox.r |  |  |
| Bretthorst (1993) difference in means, analytical solution, with package Brobdingnag for very large numbers, implementation after P Gregiory (2005) | DiM_Bretthorst_PG.r |  |  |
| example calls | DiM_Bretthorst_PG_calls.r |  |  |
| Bretthorst (1993) difference in means, analytical solution, with package Brobdingnag for very large numbers, implementation after UM Studer (1998) | DiM_Bretthorst_UMS.r |  |  |
| example calls | DiM_Bretthorst_UMS_calls.r |  |  |
|  | **External functions** |  |  |
| exact Bayesian proportion test from Sverdlov, Ryeznik & Wu (2015) | EXT_bayesian2beta.r |  |  |
| from Kruschke (2014) DBDA 2nd ed., general functions | EXT_DBDA2E-utilities.R |  |  |
| from Kruschke (2014) DBDA 2nd ed., used for PPC and heterogenuous variances | EXT_Jags-Ymet-XmetMulti-Mrobust.R |  |  |
| general functions | **all parts** |  |  |
| Cohen's d, descriptive statistics of all kinds in one table, Tukey's fivenum with labels, convert Aquad style tables to truth tables and v.v., print prime implicants from QCA objects, full distance matrix, optimal cut through a proximity matrix after Oldenbürger and plot it, plot prototypes in 2d + 3d, plot eigenvalues (MDS), correlation and p-values | ptall_generalfuncs.r |  |  |
| determine and plot beta distribution values from three quantile points via optimization, role model: Coghlan (2017) | ptall_generalfuncs_Bayes_Beta_determine.r |  |  |
| functions to apply and plot prior probability functions for successes/ failures, calculate summary statistics for posterior, HDI, formula to calculate beta posterior via conjugation, plot prior, likelihood, posterior, updated prior and update therefor posterior, summary statistics | ptall_generalfuncs_Bayes_binomial.r |  |  |
| Bayesian proportion test (e.g. successes/ failures): calculate, summarize, and plot prior from successes/ failures, convert to beta disztribution values and v.v., update binomial prior with likelihood to beta (posterior) and plot, tweaked bayes.prop.test summary from BayesianFirstAid and plot theta_diff, MCMC plot, simulation from posterior, grid approximation via brute force, exact (binomial difference) tests (Evan Miller, Chris Stucchio, the approach by Sverdlov, Ryeznik, Wu (2015) from bayesian2beta.r in a tweaked form to work with log values or brob objects and plot results, numerical integration (Simpson rule) for brob objects, brute force comparison of rbetas vs. dbetas and plot it | ptall_generalfuncs_Bayes_binomial-prop-test.r |  |  |
| useful functions for brob objects, convert a list to a vector, calculate the scalarproduct, numerical integration (Simpson rule), more numerical integration methods for brob objects can be found at https://github.com/abcnorio/R-largenum-integration | ptall_generalfuncs_brob-integral.r |  |  |
| Scientific Theory | **Part I** |  |  |
| simple true/ false statements | ptI_sciencetheory_logic.r | 2 | 2.2 |
| Classical Statistics | **Part II** |  |  |
| Lady Bristol tea taste experiment by Fisher using his methods using hypergeometric distribution, fisher.test, and manual calculation using factorials | ptII_quan_classicstats_Fisher_ladyteataste.r | 4 | 4.5.1.1 |
| calculate and plot power vs. effect sizes in accordance to different hypotheses | ptII_quan_classicstats_N-P_powerfunc.r | 4 | 4.5.2.1 |
| different views on sample differences if population parameters are known | ptII_quan_classicstats_N-P_stat-signif-isNOT-practsignif.r | 4 | 4.5.2.1 |
| reproduce Gelman & Carlin (2014), retrodesign power analysis | ptII_quan_classicstats_GandC_type-S-M-error.r | 4 | 4.5.2.2 |
| plot effect size and power for retrodesign after Gelman & Carlin (2014), plot sample distribution under H_0 and hypothesis of true effect size | ptII_quan_classicstats_GandC_type-S-M-error_helpfuncs.r | 4 | 4.5.2.2 |
| plot H_0, H_1 for one- and two-sided tests | ptII_quan_classicstats_N-P_nulldist-hypotest.r | 4 | 4.5.2.5 |
| functions to plot H_0, H_1 for hypothesis directions: two-sided, less, greater, and plot alpha and beta error rates for t-distributed densities | ptII_quan_classicstats_N-P_nulldist-hypotest_helpfuncs.r | 4 | 4.5.2.5 |
| working and simulating confidence intervals (CI) | ptII_quan_classicstats_N-P_confint.r | 4 | 4.5.2.6 |
| bootstrap CIs and other statistics via simulation, simulate difference in means, use Bayesian bootstrap | ptII_quan_classicstats_N-P_confint_bayesboot.r | 4 | 4.5.2.6 |
| CI evolution in relation to varying sample sizes | ptII_quan_classicstats_N-P_confint_errorbars.r | 4 | 4.5.2.6 |
| calculate CI for mean, difference in means, and plot them | ptII_quan_classicstats_N-P_confint_helpfuncs.r | 4 | 4.5.2.6 |
| simulate t-test with p- and t-values | ptII_quan_classicstats_N-P_confint_p-t-value.r | 4 | 4.5.2.6 |
| simulate t-tests, compare with Cohen's d | ptII_quan_classicstats_N-P_confint_p-t-value_helpfuncs.r | 4 | 4.5.2.6 |
| change of CIs along with error bars, calculate covered CIs | ptII_quan_classicstats_N-P_confint-errorbars_helpfuncs.r | 4 | 4.5.2.6 |
| simulation 'difference in means' via replicate | ptII_quan_classicstats_N-P_simulation.r | 4 | 4.5.4 |
| simulate an empirical real run of how to get all cards of a soccer card album | ptII_quan_classicstats_N-P_example-soccer-sim.r | 4 | 4.5.4.1 |
| simulate how many cards are still to go using chance and not commone sense like card exchange, etc. | ptII_quan_classicstats_N-P_example-soccer-sim_helpfuncs.r | 4 | 4.5.4.1 |
| dependence of sample size characteristics via simulated data (N, SE, t-value, Cohen's d, ...) | ptII_quan_classicstats_N-P_SE-N-dep.r | 4 | 4.5.5 |
| all the non-sense around the Null ritual ie. the NHST procedure | ptII_quan_classicstats_nullritual.r | 4 | 4.5.7 |
| plot NHST decision making using acceptance/ rejection regions, alpha and beta error rates | ptII_quan_classicstats_NHST_nulldist.r | 4 | 4.5.8 |
| differences of normal vs. mixed normal distribution in accordance to plot and statistics, simulation H_0 | ptII_quan_classicstats_pvalue-as-base.r | 4 | 4.5.8.1 |
| relationship of normal and t distribution | ptII_quan_classicstats_normal-vs-t.r | 4 | 4.5.8.2 |
| simulate central limit theorem (CLT) und varying conditions | ptII_quan_classicstats_centrallimittheorem.r | 4 | 4.5.8.3 |
| basic functions to simulate CLT | ptII_quan_classicstats_centrallimittheorem_helpfuncs.r | 4 | 4.5.8.3 |
| relationship of N, t-, and p-value, simulate with same seed, growing sample sizes, constant t-value, and post-hoc power simulation | ptII_quan_classicstats_p-t-df-relationship.r | 4 | 4.5.9 |
| relate N, p-value, Cohens' d, odds ratio, and risk ratio with each other graphically | ptII_quan_classicstats_effectsizes.r | 4 | 4.6.10 |
| function to relate N, p-value, and Cohen's d | ptII_quan_classicstats_effectsizes_helpfuncs.r | 4 | 4.6.10 |
| taken and twekaed from package sjtats as it disappeared from the package, convert odds ratio to risk ratio | ptII_quan_classicstats_effectsizes_helpfuncs_sjstats.r | 4 | 4.6.10 |
| investigate Simpson Paradox with classical UCB admission data set using plots, tables, corrected base rates, and glmer model, simulate Simpson Paradox data and detect sub-groups with Simpsons from package Simpsons | ptII_quan_classicstats_Simpsonparadox.r | 4 | 4.6.11.1 |
| function to plot two groups to detect Simpson Paradox | ptII_quan_classicstats_Simpsonparadox_helpfuncs.r | 4 | 4.6.11.1 |
| Jeffrey's-Lindley's paradox with data examples | ptII_quan_classicstats_JeffreysLindleyparadox.r | 4 | 4.6.11.2 |
| simulate p-hacking with varying sample sizes for standard alpha error rates, comparison with a priori power analysis, and plot results | ptII_quan_classicstats_p-hacking-sim.r | 4 | 4.6.2 |
| function to simulate linear model with sim from package arm as well as p-hacking methods | ptII_quan_classicstats_p-hacking-sim_helpfuncs.r | 4 | 4.6.2 |
| calculate, simulate, and plot power under varying models | ptII_quan_classicstats_dealingwithpower.r | 4 | 4.6.3 |
| apply R-Index and TIVA after Schimmack and colleagues, e.g. on Bem's clairvoyance study and calculate z-curve | ptII_quan_classicstats_R-index_z-curve.r | 4 | 4.6.4.1 |
| function to calculate R-Index, p-values for R-Index, Test of Insufficient Variance (TIVA) after Schimmack and colleagues | ptII_quan_classicstats_R-index_z-curve_helpfuncs.r | 4 | 4.6.4.1 |
| apply function to calculate variance estimation | ptII_quan_classicstats_varianceestimation.r | 4 | 4.6.5 |
| function to calculate variance estimation if sigma is known or unknown and plot it | ptII_quan_classicstats_varianceestimation_helpfuncs.r | 4 | 4.6.5 |
| apply different randomization strategies to create samples | ptII_quan_classicstats_randomization.r | 4 | 4.6.6 |
| handling missing data and associated changes with and without imputation methods, comparison of methods of imputation with each other and with brms (Bayesian linear model) | ptII_quan_classicstats_missingdata.r | 4 | 4.6.7 |
| comparison with Bland-Altman stats and plot (mean-diff plot), TOST from package TOSTER, Pitman-Morgan Test, tests from packages BayesFactor, BEST, usage of implementation of GL Bretthorst (1993) 'On the difference in mean', using e.g. original data from Bland-Altman | ptII_quan_classicstats_equivalentmethods.r | 4 | 4.6.8.1 |
| mean-difference plot after Tukey (equals Bland-Altman plot) | ptII_quan_classicstats_equivalentmethods_helpfuncs.r | 4 | 4.6.8.1 |
| determine variance homogeneity via various tests (Levene, Breusch-Pagan, non-constant error variance, Cook and Weisberg, Koenker's studentized version of test statistic) | ptII_quan_classicstats_variancehomogeneity.r | 4 | 4.6.8.2 |
| investigation of residuals from linear models for various characteristics (normality, skewness, kurtosis, various sample sizes | ptII_quan_classicstats_normaldist_residuals.r | 4 | 4.6.9.1 |
| impact and consequences of influential points and outliers, calculate linear model, t-test, outlier test, correlation test, leverage plots | ptII_quan_classicstats_outliers-and-influentialpoints.r | 4 | 4.6.9.3 |
| function to plot outlier (with, without) via regression lines | ptII_quan_classicstats_outliers-and-influentialpoints_helpfuncs.r | 4 | 4.6.9.3 |
| Exploratory Data Analysis (EDA) sensu JW Tukey | **Part II** |  |  |
| compare median vs. mean, show robust plots of data, apply lm vs. rlm on empirical data | ptII_quan_EDA_intro_overviewrobust.r | 5 | 5.2.-5.3. |
| function to plot residuals (lm vs. rlm), simulate and plot median/ mean from normal distribution | ptII_quan_EDA_intro_overviewrobust_helpfuncs.r | 5 | 5.2.-5.3. |
| conversion to log values lead to straight lines using a real life example from German states population characteristics, compare to classical linear model | ptII_quan_EDA_case_German-states-population.r | 5 | 5.5.1 |
| analyzing fertility in Swuisse between Catholics (yes, no, something else) using only descriptive plots and correlations, use subgroups to understand data | ptII_quan_EDA_case_Suisse-fertility.r | 5 | 5.5.2 |
| function to plot two variables with a continuous index | ptII_quan_EDA_case_Suisse-fertility_helpfuncs.r | 5 | 5.5.2 |
| investigate the famous iris data with pairs, lda, and tables | ptII_quan_EDA_case_Anderson_iris-species-in-biology.r | 5 | 5.5.3 |
| use only descriptive statistics, tables, and plots to investigate the survivors' characteristics of the Titanic and how to survive and why, answer certain questions about the conditions of surviving or dying during the Titanic catastrophe | ptII_quan_EDA_case_Titanic_death-and-dying.r | 5 | 5.5.4.6 |
| function to plot densities for various subgroups | ptII_quan_EDA_case_Titanic_death-and-dying_helpfuncs.r | 5 | 5.5.4.6 |
| analyze data on leadership in Spain (educational context) with distance methods (HCA, MDS), prototype analysis, heatmap, corrgram, levelplot, and simple descriptive plots | ptII_quan_EDA_case_Spain_leadership-in-education.r | 5 | 5.5.5 |
| tweaked version of heatmap from package heatmap.plus (not available in R v.4) | ptII_quan_EDA_case_Spain_leadership-in-education_helpfuncs.r | 5 | 5.5.5 |
| experiment in chiropractice about heart rate variability using linear models without any significance test, learning from sample size characteristics, use histograms and interaction plots | ptII_quan_EDA_case_Chiro_heartrate-variability.r | 5 | 5.5.6 |
| function to plot interactions | ptII_quan_EDA_case_Chiro_heartrate-variability_helpfuncs.r | 5 | 5.5.6 |
| Bayesian Statistics | **Part II** |  |  |
| calculate beta posterior from prior and likelihood (conjugation, grid approximation), influence of priors, various shapes of beta | ptII_quan_Bayes_Beta-distribution.r | 6 | 6.12 |
| determine beta distribution from three quantile points with/ without optimization | ptII_quan_Bayes_find-Beta-distribution-shapeparams.r | 6 | 6.12 |
| plot prior, likelihood, and posterior of the empirical data | ptII_quan_Bayes_Fisher_LadyBristol-Beta-disttribution.r | 6 | 6.12 |
| display Gamma function for different parameters | ptII_quan_Bayes_Gamma-distribution.r | 6 | 6.12 |
| different priors (normal with different sigmas, Cauchy, uniform) | ptII_quan_Bayes_regularization.r | 6 | 6.12 |
| display how prior and likelihood shape the posterior, see different influences on posterior | ptII_quan_Bayes_Prior-Likeli-Post_relationship.r | 6 | 6.12.1 |
| simulate binormial and normal distributions | ptII_quan_Bayes_MC-simulation_binom-norm.r | 6 | 6.13 |
|  calculate and plot a random walk as 2d + 3d | ptII_quan_Bayes_RandomWalk.r | 6 | 6.13 |
| function to calculate a simple random walk | ptII_quan_Bayes_RandomWalk_helpfuncs.r | 6 | 6.13 |
| apply pi simulation for varying sample sizes | ptII_quan_Bayes_simulate-pi.r | 6 | 6.13 |
| simple function to calculate pi and plot it (inefficient, but works) | ptII_quan_Bayes_simulate-pi_helpfuncs.r | 6 | 6.13 |
| local minima using Himmelblau's function, plot in 3d | ptII_quan_Bayes_problem-local-minima.r | 6 | 6.13.1.4 |
| apply Hamilton Monte Carlo (HMC) algorithm, plot and investigate (bivariate normal distribution), use packages hmclearn, rethinking, and bayesplot as well as the basic HMC algorithm (Neal, 2011, chap. 5), analyze MCMC chains via plots and typical diagnostics (e.g. Gelman, Heidelberger-Welch, effective sample size, remove burn-in, ... ), compare with MH algorithm | ptII_quan_Bayes_HMC.r | 6 | 6.13.2.3.1 |
| functions to simulate bivariate normal distribution (rnorm, mvrnorm), calculate gradient for HMC algorithm, simulate bivariate normal distribution via HMC after McElreath (2015), describe MCMC chains: Heidelberger-Welche diagnostics, describe development of mean and covariance, adjust limits to let a comparison value appear on a posterior plot, and simulate bivariate normal distribution via MH algorithm | ptII_quan_Bayes_HMC_helpfuncs.r | 6 | 6.13.2.3.1 |
| simulate mean of normal distribution with Metropolis-Hastings (MH) algorithm, investigate MCMC chains using package coda, apply on empirical data, investigate acceptance/ rejection rates, compare MCMC chains, posterior predictive distribution and plots | ptII_quan_Bayes_MetropolisHastings_example-normdist.r | 6 | 6.13.4.1 |
| functions to perform MH algorithm to simulate normal distribution based on empirical data, priors, Gibbs sampler, plot MCMC and its parts, normal posterior predictive distribution | ptII_quan_Bayes_MH-Gibbs_example_helpfuncs.r | 6 | 6.13.4.1 |
| simulate mean posterior via Gibbs sampling, analyze posterior | ptII_quan_Bayes_GibbsSampling_example-normdist.r | 6 | 6.13.4.2 |
| simulate mean of a normal distribution using JAGS, investigate MCMC, compare with package Bolstad | ptII_quan_Bayes_JAGS_example-norm.r | 6 | 6.13.4.3 |
| analyze Lady Bristol's tea data with classical and Bayesian methods (analytical solution and MCMC with JAGS, package BEST, BUGS) | ptII_quan_Bayes_Fisher_LadyBristol-BUGS.r | 6 | 6.13.5 |
| function to analyze Lady Bristol experiment with Bayes Theorem, plot successes/ failures, HDI, MAP, and run BUGs model with package RBugs from R |  ptII_quan_Bayes_Fisher_LadyBristol-BUGS_helpfuncs.r | 6 | 6.13.5 |
| calculate and plot Boltzmann/ Shannon entropy using a (not-so-fair/ fair) coin and dice | ptII_quan_Bayes_MaximumEntropy.r | 6 | 6.14 |
| functions to reproduce Jaynes' (1962) analyses, entropy simulation after McElreath (2015, p.277) | ptII_quan_Bayes_MaximumEntropy_helpfuncs.r | 6 | 6.14 |
| apply entropy functions on word counts | ptII_quan_Bayes_Entropy_KullbackLeibler.r | 6 | 6.14.1 |
| functions to calculate H (Shannon entropy) from counts and priors or Kullback-Leibler distance| | ptII_quan_Bayes_Entropy_KullbackLeibler_helpfuncs.r | 6 | 6.14.1 |
| perform and plot Jaynes' (1962) analysis on a (fair?) dice | ptII_quan_Bayes_MaximumEntropy_Jaynes-fair-dice.r | 6 | 6.14.3 |
| functions to calculate entropy of a fair or not-so-fair coin and simulate a dice | ptII_quan_Bayes_MaximumEntropy_Jaynes-fair-dice_helpfuncs.r | 6 | 6.14.3 |
| prepare dataset, remove NAs, EDA plots, classical vs. Bayesian statistics with Bayesian binomial test, MCMC diagnostics, test difference in means via classical and Bayesian solution incl. HDI, ROPE, bayesboot, plot results | ptII_quan_Bayes_case_presidential-heights.r | 6 | 6.15.1 |
| tweaked function of bayes.binom.test from package BayesianFirstAid, function to calculate posterior OR/RR | ptII_quan_Bayes_case_presidential-heights_helpfuncs.r | 6 | 6.15.1 |
| compare developments and extreme cases of successes/ failures and resulting posterior probs | ptII_quan_Bayes_case_startagain-successrates.r | 6 | 6.15.2 |
| show that long-term analysis over all years vs. sequential analysis per year (prior becomes posterior becomes prior, etc.) leads exactly to the same concluding results within the Bayesian approach | ptII_quan_Bayes_case_startagain-successrates-longterm.r | 6 | 6.15.2.1 |
| frequentist, chi-square, power, JAGS, posterior odds, plots, MCMC brute force variants, test and plot hypotheses, grid approximation, exact tests, MAP, 2d + 3d plots of theta values over intgral, comparison of methods, using tweaked functions from appell, tolerance, bayesian2beta.r, comparison with package BayesFactor and Bretthorst approach| | ptII_quan_Bayes_case_presidential-debates.r | 6 | 6.15.3 |
| simple application of Bayes Theorem, using discrete values, and shift from prior knowledge to posterior to prior, etc. ie. update Bayes Theorem with new information | ptII_quan_Bayes_intro-BayesTheorem_tea.r | 6 | 6.2.2.4 |
| apply Bayes Theorem on medical tests using prevalence (of a disease in population), sensitivity (1-false_positives), and specifity (1-false_negatives) | ptII_quan_Bayes_intro-BayesTheorem_medicaldiagnosis.r | 6 | 6.2.3.4 |
| function to apply Bayes Theorem for medical tests using prevalence p(A), sensitivity p(B|A), and specifitiy p(not-B|not-A)| | ptII_quan_Bayes_intro-BayesTheorem_medicaldiagnosis_helpfuncs.r | 6 | 6.2.3.4 |
| calculate probability of a covid-19 test in relation to population characteristics | ptII_quan_Bayes_intro-BayesTheorem_covid19-test.r | 6 | 6.2.4 |
| analyse posterior and MCMC chains (convergence), apply on empirical data set | ptII_quan_Bayes_posterior.r | 6 | 6.5.1 |
| posterior distribution of a mean with different informed priors using package Bolstad | ptII_quan_Bayes_simple-estimation-mean-post.r | 6 | 6.5.1 |
| function to plot mean posterior with prior and likelihood, tweaked normgcp from package Bolstadt (bug fix) | ptII_quan_Bayes_simple-estimation-mean-post_helpfuncs.r | 6 | 6.5.1 |
| empirical example from Kruschke's remarks (BFs are not enough to interpret results), better is to use priors and posterior probs, example: huge BF but roughly same parameters of the linear model, BF only caused by different priors | ptII_quan_Bayes_BayesFactors_test-hypos.r | 6 | 6.7.1 |
| show loss function in the context of an empirical example (success rates in drug therapy) to draw conclusions for actual work | ptII_quan_Bayes_lossfun_startagain.r | 6 | 6.7.1.2 |
| compare and simulate Bayes Factors (BFs) for various sample sizes, t-values, determine minimal BF with different approaches | ptII_quan_Bayes_BayesFactors_dependence-on-N-sim.r | 6 | 6.7.1.3 |
| function to simulate BFs in relation to N, convert p-values to BFs, relate p-values and sample sizes | ptII_quan_Bayes_BayesFactors_dependence-on-N-sim_helpfuncs.r | 6 | 6.7.1.3 |
| function to calculate t-test with BF, then prepare, describe, analyze empirical data using methods for binomial data and difference in means (classical and Bayesian solution), anova with repeated measurement vs. brms | ptII_quan_Bayes_case_exp-extra-sensual-perception.r | 6 | 6.7.1.4 |
| some artificial but meaningful calculations along with a very critical discussion of the theoretical background and design of the Bem study on clairvoyanca | ptII_quan_Bayes_Bem-study-aspects.r | 6 | 6.7.1.5 |
| apply BFs in the context of EDA plots, t-tests, Cohen's d, linear models, and a real full Bayesian approach with brms, and other criteria (Bayes R^2, loo, waic, kfold), different results if a categorial variable is made continuous (ie. converted back to its natural original state like categorial age) | ptII_quan_Bayes_BayesFactors_further-remarks.r | 6 | 6.7.1.6 |
| apply function to get information criteria from a linear model (empirical example) | ptII_quan_Bayes_information-criteria.r | 6 | 6.7.2 |
| function to calculate and extract information criteria from linear models | ptII_quan_Bayes_information-criteria_helpfuncs.r | 6 | 6.7.2 |
| examples to show under- and overfitting incl. polynomials, use Kullback-Leibler (KL) divergence between probs | ptII_quan_Bayes_over-and-underfitting.r | 6 | 6.7.3 |
| plot HDI and CI for various data sets to demonstrate the qualitative difference of those interval types | ptII_quan_Bayes_HDI.r | 6 | 6.7.4.1 |
| function to plot HDI vs. symmetric CI | ptII_quan_Bayes_HDI_helpfuncs.r | 6 | 6.7.4.1 |
| show importance of ROPE vs. simple clear-cut for decision-making (difference in means). plot with HDI, Bayesian binomial test for various values, priors, and hypotheses, priors and their influence on BFs, apply on empirical data, apply and plot with package BEST, analyze MCMC and posterior probs | ptII_quan_Bayes_ROPE-BayesFactor.r | 6 | 6.7.4.2 |
| function to demonstrate change in BF due to prior believes | ptII_quan_Bayes_ROPE-BayesFactor_helpfuncs.r | 6 | 6.7.4.2 |
| simulate PPC via JAGS using successes/ failures and plot it | ptII_quan_Bayes_PPC_model-check.r | 6 | 6.7.4.4 |
| function to calulate one-/two-sided test (PPC) | ptII_quan_Bayes_PPC_model-check_helpfuncs.r | 6 | 6.7.4.4 |
| demonstrate PPC for homogenous variances (yes/no), plot and test specific hypotheses (classical, Bayesian statistics with brms), treatment/ control group design, compare linear models | ptII_quan_Bayes_PPC_model-check-graph.r | 6 | 6.7.4.5 |
| perform analysis (e.g. with JAGS) and plots along with posterior predictive checks (PPC) incl. diagnostic MCMC plots, bootstrap using full cases or real values, exercise Kruschke (2014, chap 18.3) regarding heteroscedasticity (influence of un-/equal variances between groups) | ptII_quan_Bayes_case_wordcounts-PPC.r | 6 | 6.7.4.6 |
| functions to prepare and summarize MCMC chains and posteriors from Kruschke (2014) scripts to match requirements here for PPC | ptII_quan_Bayes_case_wordcounts-PPC_helpfuncs.r | 6 | 6.7.4.6 |
| Qualitative Data Analysis | **Part III** |  |  |
| example for usage of expand.grid | ptIII_qual_code-paradigm_table-analysis.r | 9 | 9.4 |
| prepare (e.g. removal of redundant parts, special characters, white space, punctuations, stop words and convert to lower cases, split text, word cloud, plot frequencies, KWIC, collocation, ...) and analyze text using packages stringi, SnowbalC, tm, magrittr, quanteda, corps2, etc., use wordstems, corpus inspection, ... | ptIII_qual_quan-textanalysis.r | 10 | 10.1 |
| simple true/ false statements | ptIV_qual_Boole_basics.r | 12 | 12.1 |
| analyse data set from Krook (2010) using QCA, extract prime implicants, check for consistencies, prepare positive and negative outcome for general discussion| | ptIV_qual_Boole_case_Krook_women-in-parliament.r | 12 | 12.11.1 |
| apply QCA on Titanic data set to investigate a minimal set for survival (yes/ no) | ptIV_qual_Boole_case_Titanic_death-and-dying.r | 12 | 12.11.2 |
| single case demonstration how Boolean minimization actually works | ptIV_qual_Boole_logical-minimization.r | 12 | 12.3 |
| artificial data set to demonstrate QCA for negative and positive outcome | ptIV_qual_Boole_case_school-success.r | 12 | 12.6 |
| simple plot to demonstrate what fuzzy logic is | ptIV_qual_Boole_fuzzy-logic.r | 12 | 12.7 |
| Mixed Methods | **Part IV** |  |  |
| one-liner to get prime numbers using Euclidean mod division | ptV_mixed_prime-numbers.r | 13 | 13.3.1 |
